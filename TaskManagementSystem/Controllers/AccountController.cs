    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (user != null)
            {
                ...
                {
                    ModelState.AddModelError("", "Администратор не подтвердил ваш аккаунт");
                }
            }
            return View(model);
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, isConfirmed = false };
        ...
        public async Task<ActionResult> BrowseUsers()
        {
            AdminUserRolesViewModel model = new AdminUserRolesViewModel();
            ApplicationDbContext db = new ApplicationDbContext();
            List<KeyValuePair<ApplicationUser, List<string>>> usersRoles = new List<KeyValuePair<ApplicationUser, List<string>>>();
            var users = UserManager.Users.ToList();
            foreach(var usr in users)
            { 
            var roles = await UserManager.GetRolesAsync(usr.Id);
            usersRoles.Add(new KeyValuePair<ApplicationUser, List<string>>(usr,roles.ToList()));
            }
            model.Users = usersRoles;
            return View(model);
        }
        [Authorize(Roles = "Admin")]
        [HttpGet]
